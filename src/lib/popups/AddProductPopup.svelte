<script>
    ...
    let genetics = $state(props.data?.geentics || "")
    let category = $state(props.data?.category || "")
    let type = $state(props.data?.type || "")
    let price = $state(props.data?.price || 0)
    let stock = $state(props.data?.stock || 0)
    let thc = $state(props.data?.thc || 0)
    let cbd = $state(props.data?.cbd || 0)
    let aspects = $state(props.data?.aspects?.map(val => val.name).join(';') || "")
    let idealFor = $state(props.data?.idealfors?.map(val => val.name).join(';')  || "")
    let flavors = $state(props.data?.flavors?.map(val => val.name).join(';') || "")
    let effects = $state(props.data?.effects?.map(val => val.name).join(';') || "")
    ...
    const isDisabled = () => {
        if(!image || !name || !description || !genetics || !category || !type || price == null || stock == null || !thc || !cbd || !aspects || !idealFor || !flavors || !effects) return true
        if(props.data && (
            ...
            thc === props.data.thc && 
            cbd === props.data.cbd &&
            props.data.aspects.every((val) => aspects.includes(val)) &&
            props.data.idealfors.every((val) => idealFor.includes(val)) &&
            props.data.aspects.every((val) => flavors.includes(val)) &&
            props.data.aspects.every((val) => effects.includes(val))
        )) return true
        else false
    }
    ...
    const save = () => {
        if(props.save) {
            const product = {
                ...
                thc_rate:Number(thc),
                cbd_rate:Number(cbd),
                aspects:aspects.split(';').map((val) => {return {name:val}}),
                idealfors:idealFor.split(';').map((val) => {return {name:val}}),
                flavors:flavors.split(';').map((val) => {return {name:val}}),
                effects:effects.split(';').map((val) => {return {name:val}}),
            }
...
<div class="AddProductPopup">
    <PopupContent title={props.data ? "admin.faq.edit-popup" : "admin.faq.add-popup"} submitDisabled={isDisabled()} submitLabel={props.data ? 'commons.validate':'commons.add'} onSubmitClick={save}>
        <div class="AddProductContainer">
            <Uploader title={'admin.product.image-title'} setImage={(img)=>{image = img}}/>
