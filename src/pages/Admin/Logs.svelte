<script>
    ...
    const deleteElement = (data) => {
        openPopup(ConfirmPopup, {
            onSubmitClick:()=>{
                Request.delete('/log/' + data.id).then(res => {
                    fetchElements()
                }).catch(err => {
                    console.log(err)
                })
    ...
    const columns = [
        ...
        {size:150, isFixed : true, label:"admin.logs.method", id:"method", type:"text"},
        {size:50, isFixed : false, label:"admin.logs.url", id:"url", type:"text"},
        {size:50, isFixed : false, label:"admin.logs.user-id", id:"user_id", type:"text"},
